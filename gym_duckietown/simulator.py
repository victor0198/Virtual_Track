...
from .objects import WorldObj, DuckieObj, TrafficLightObj, DuckiebotObj, BBObj
...
sys.path.append("..")
from gym_duckietown.object_detection.utils import ops as utils_ops
...
# Distance (diameter) between the center of the robot wheels (9cm)
WHEEL_DIST = 0.09

# Total robot width at wheel base, used for collision detection
# Note: the actual robot width is 21cm, but we add a litte bit of buffer
...
class Simulator(gym.Env):
    ...
    def reset(self):
        ...
        for _ in range(MAX_SPAWN_ATTEMPTS):
            ...
            x = self.np_random.uniform(i, i + 0.01) * self.road_tile_size
            z = self.np_random.uniform(j, j + 0.01) * self.road_tile_size
        ...
        for obj_idx, desc in enumerate(map_data.get('objects', [])):
            ...
            if static:
                if kind == "traffic_light":
                    ...
                    obj = BBObj(obj_desc, self.domain_rand, SAFETY_RAD_MULT)
                else:
    ...
    def _inconvenient_spawn(self, pos):
        ...
        results = [np.linalg.norm(x.pos - pos) <
                   ...
                   for x in self.objects if x.visible and x.kind != 'bb'
                   ]
    ...
    def _valid_pose(self, pos, angle, safety_factor=1.0):
        ...
        l_pos = pos - (0.5 * ROBOT_WIDTH) * r_vec - (0.295 * ROBOT_LENGTH) * f_vec
        r_pos = pos + (0.5 * ROBOT_WIDTH) * r_vec - (0.295 * ROBOT_LENGTH) * f_vec
        f_pos = pos + (0.295 * ROBOT_LENGTH) * f_vec
        ...
        # front sensor
        zones_front = get_agent_f_zones(pos, angle)
        global covered_front_zones
        covered_front_zones = 0
        for zone in zones_front:
            if self._collision(zone):
                covered_front_zones += 1

        # back sensors
        zones_rear = get_agent_r_zones(pos, angle)
        global covered_rear_zones
        covered_rear_zones = 0
        for zone in zones_rear:
            if self._collision(zone):
                covered_rear_zones += 1
    ...
    def update_physics(self, action, delta_time=None):
        ...
        f_vec = get_dir_vec(self.cur_angle)
        self.cur_pos = self.cur_pos + 0.295 * ROBOT_LENGTH * f_vec
        ...
        for obj in self.objects:
            if not obj.static and obj.kind == "duckiebot":
                ...
                obj.step(delta_time, self.cur_pos, self.cur_angle)
            else:
                obj.step(delta_time)
    ...
    def _render_img(self, width, height, multi_fbo, final_fbo, img_array, step, type, top_down=True):
        """
    ...
    def draw_detection_area(self, img):
        ...
        cv2.line(img, (600, 0), (600, 200), (0, 0, 250), 2)
        cv2.line(img, (600, 200), (300, 200), (0, 0, 250), 2)
        cv2.line(img, (300, 200), (300, 0), (0, 0, 250), 2)
        return img

    def draw_car(self, img):
        cv2.line(img, (513, 51), (517, 51), (0, 0, 250), 2)
        cv2.line(img, (513, 51), (513, 59), (0, 0, 250), 2)
        cv2.line(img, (513, 59), (517, 59), (0, 0, 250), 2)
        cv2.line(img, (517, 51), (517, 59), (0, 0, 250), 2)
        return img

    def draw_front_sensor(self, img):
        global covered_front_zones
        lines = [[10, 10, 20, 10], [9, 15, 21, 15], [8, 20, 22, 20], [7, 25, 23, 25], [6, 30, 24, 30], [5, 35, 25, 35]]
        colors = [(200, 200, 0), (210, 160, 0), (220, 120, 0), (230, 80, 0), (240, 40, 0), (250, 0, 0)]
        movex = 500
        movey = 50
...
def get_agent_f_zones(pos, angle):
    zones = []
    # detected distances in cm
    distances = [5, 10, 15, 20, 25, 30]

    for distance in distances:
        zones.append(make_f_zone(
            pos,
            ROBOT_WIDTH,
            ROBOT_LENGTH,
            get_dir_vec(angle),
