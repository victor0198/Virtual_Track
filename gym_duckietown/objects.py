...
class BBObj:
    ...
    def generate_geometry(self):
        ...
        self.obj_corners = generate_corners(self.pos,self.min_coords, self.max_coords, self.angle, self.scale)
    ...
    def step(self, delta_time, cur_pos, cur_angle):
        """
        Use a motion model to move the object in the world
        """
        self.pos[0] = cur_pos[0]
        self.pos[2] = cur_pos[2]
        self.angle = cur_angle
        self.y_rot = 180 + cur_angle * 180 / np.pi
...
class DuckieObj(WorldObj):
    def __init__(self, obj, domain_rand, safety_radius_mult, walk_distance):
        ...
        self.walk_distance = walk_distance + 0.1 * walk_distance

        # Dynamic duckie stuff

        # Randomize velocity and wait time
        if self.domain_rand:
            self.pedestrian_wait_time = np.random.randint(3, 5)
        ...
        self.wiggle = np.random.choice([90, 45, 30], 1)
...
class TrafficLightObj(WorldObj):
    def __init__(self, obj, domain_rand, safety_radius_mult):
        ...
        self.texs = [
            load_texture(get_file_path("textures", "TL0", "png")),
            load_texture(get_file_path("textures", "TL1", "png")),
            load_texture(get_file_path("textures", "TL2", "png")),
            load_texture(get_file_path("textures", "TL1", "png")),
        ]
    ...
    def step(self, delta_time):
        ...
        if self.pattern in [1, 3]:
            self.freq = 2
        # green light duration
        if self.pattern == 2:
            self.freq = 6

        if time() - self.time > self.freq:  # Swap patterns
