...
def agent_boundbox(true_pos, width, length, f_vec, r_vec):
    ...
    corners = np.array([
        true_pos - hwidth * r_vec - hlength * f_vec + 0.13 * f_vec,
        true_pos + hwidth * r_vec - hlength * f_vec + 0.13 * f_vec,
        true_pos + hwidth * r_vec + hlength * f_vec + 0.13 * f_vec,
        true_pos - hwidth * r_vec + hlength * f_vec + 0.13 * f_vec
    ])[:, [0, 2]]
...
def make_f_zone(true_pos, width, length, f_vec, r_vec, distance):
    # halfwidth/length
    hwidth = 0.5 * width
    hlength = 0.5 * length
    ...
    zone_corners = np.array([
        true_pos - hwidth * r_vec + hlength * f_vec + 0.13 * f_vec,
        true_pos + hwidth * r_vec + hlength * f_vec + 0.13 * f_vec,
        true_pos + hwidth * r_vec + hlength * f_vec + 0.13 * f_vec + 0.01 * f_vec * distance,
        true_pos - hwidth * r_vec + hlength * f_vec + 0.13 * f_vec + 0.01 * f_vec * distance
    ])[:, [0, 2]]
...
def make_r_zone(true_pos, width, length, f_vec, r_vec, distance):
    # halfwidth/length
    hwidth = 0.5 * width
    hlength = 0.5 * length
